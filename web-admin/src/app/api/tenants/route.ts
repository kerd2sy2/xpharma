...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, slug, address = '', contact_phone = '', logo_url = '', category = 'مخزن أدوية' } = body;
    ...
    // 1. Insert tenant in public.tenants with address, contact_phone, logo_url, category
    const insertTenant = await query(
      `INSERT INTO public.tenants (name, slug, schema_name, status, api_key_hash, address, contact_phone, logo_url, category)
      ...
      [
        ...
        category ? category.trim() : 'مخزن أدوية',
      ]
    );
